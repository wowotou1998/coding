...
def crawl_for_info(Rtext,baseinfoLIST,jobinfoLIST, csvFile):

        # -----------------------------------------------------------------------------编码问题的最佳解决方法----------------------上知乎搜索----------------------------------------
        # csvFile3 = open('E:\\info.csv', 'w+', encoding='UTF-8', newline='')
        ...
        for div in divs[0].contents:
            if div.name == 'font':
                ...
                n = str(div.get_text)
                # 正则表达式强力去空格\s|(\u00A0)|(\u3000)
                info_list = re.findall(r'[:：][\s]?[\u4e00-\u9fa5\w\s：]{4,21}', n, re.M)   # re.M表示正则匹配以每行开头重新匹配
                for arg in info_list:
                    new_arg = re.sub(p, '', arg)
                    baseinfoLIST.append(new_arg)  # baseinfoLIST下标0-3分别为举办日期，时间，校区，教室,说明（说明不常用）--------------------------------------------------------
                     #  writer2.writerow(baseinfoLIST),放在后面合并一起写
...
def main():
    ...
    for counter in range(endPage):
        ...
        for LIST_enterprise_arg in LIST_enterprise:
            SECOND_URL = 'http://job.ahut.edu.cn/cms/view/wz!viewZph.action?id={}'.format(LIST_enterprise_arg)
           # print(SECOND_URL)
            text = getHTMLtext(SECOND_URL, '')
            crawl_for_info(text, BaseLIST, JobLIST, file_target)
