...
def crawl_for_info(Rtext):
        ...
        jobinfoLIST =[]
        for i in range(j):
            jobinfoLIST.append([])  # 这一步很重要啊
            tr_tags=trs_employe_info[i] # 这表示现在在第几个tr标签中
            for td_arg in tr_tags('td'):
                jobinfoLIST[i-1].append(td_arg.string)    # 这里有5个属性，分别是序号，岗位名称，专业，招聘人数，学历要求

        baseinfoLIST.append(jobinfoLIST)

        return baseinfoLIST
        # for temp in baseinfoLIST:
        #     print(temp)
# 第一步，得到每个页码的链接-------------------------------------------------------------
...
def main():
    ...
    for counter in range(endPage):
        ...
        LIST_enterprise = crawl_for_list_twenty_info(FIRST_URL) # 得到每个企业招聘信息的单页链接
        for i in range(20):
            SECOND_URL = 'http://job.ahut.edu.cn/cms/view/wz!viewZph.action?id={}'.format(LIST_enterprise[i])
            print("正在爬取第{}页中的第{}个企业信息".format(counter+1,i+1))
            text = getHTMLtext(SECOND_URL, '')
            LIST = crawl_for_info(text)

            templist1 =[0,4,5,7,8,9,10,11]
            templist2 =[0,2,3,4]
            ws1 = wb["总库"]
            ji=2
            for baseinfotip in range(8):
                #            行        列
                ws1.cell(row=ji+i, column=ji+baseinfotip, value=LIST[templist1[baseinfotip]])
                for jobtip in range(LIST[12]):
                    ws1.cell(raw=ji+i+jobtip, column=ji+baseinfotip+jobtip,value=LIST[12][templist2[jobtip]])

        wb.save("E:\\jywcopy.xlsx")
